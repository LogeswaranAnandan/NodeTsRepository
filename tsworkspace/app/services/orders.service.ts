...
export class OrdersService {
    ...
    async wcsRequest(orderId?: number) {
        ...
        try {
            var orderJsonResponse = await this.httpUtil.makeHttpRequest(ORDER_URL, GET_METHOD, null); //productId = 12392, 11467, 12440
            ...
            for (var i = 0; orderJsonResponse.orderItem[i] != null; i++) {
                ...
                const orderItemJsonResponse = await this.httpUtil.makeHttpRequest(`https://192.168.35.129/wcs/resources/store/10201/productview/byId/${productId}?catalogId=10052`, GET_METHOD, null);
            ...
            response.responseBody = "Invalid order Id";
            return response;
        }
    }
